...
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq *request) {
  ...
  if (difftime(now, request->sent) >= 1.0) {
    if (request->times_sent >= 5) {
      ...
      while (packet) {
        send_icmp_message(sr, packet->buf, packet->len, packet->iface, 3, 1);
        packet = packet->next;
      }
      ...
      if (iface) {
        sr_send_arp_request(sr, iface, request->ip);
      } else {
        fprintf(stderr, "Error: Interface %s not found for ARP request.\n",
