...
int is_packet_valid(uint8_t *packet /* lent */,
  ...
  if (eHdr->ether_type == htons(ethertype_arp)) {
    ...
    if (len < cumulative_sz) {  // Changed from >= to <
    ...
    if (ip_cksum(ipHdr, sizeof(sr_ip_hdr_t)) != ipHdr->ip_sum) {  // Changed == to !=
    ...
    if (ipHdr->ip_p == ip_protocol_icmp) {
      ...
      if (icmp_cksum(icmpHdr, len - icmpOffset) != icmpHdr->icmp_sum) {  // Changed == to !=
